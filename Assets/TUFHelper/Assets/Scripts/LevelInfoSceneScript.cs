...
public class LevelInfoSceneScript : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        string vId = GetVideoId(currentLevelInfo.vidLink);
        if (vType.Equals("YOUTUBE"))
        {
            StartCoroutine(RequestYoutubeVideo(GetVideoId(currentLevelInfo.vidLink)));
        } else if(vType.Equals("BILIBILI")) 
        {
            StartCoroutine(RequestBilibiliVideo(GetVideoId(currentLevelInfo.vidLink)));
        }
